...
if [ "${file##*.}" = "txt" ];then
	...
	python scripts/get_seqmap_latest.py $fn'_'$line'_seqmap_exon_prom.out' sgRNA_off.out sgRNA.out $num $outnum >tmp.txt
	...
	line=$(echo $file | cut -d . -f1) 
        ...
        python scripts/get_seqmap_latest_ffq.py $line'_seqmap_exon_prom.out' $line'_sgRNA_off.out' $line'_sgRNA.out' $num $outnum >tmp.txt
